...
def main(base_output, *cur_outputs):
    ...
    default_min_flex = 5  # Defaul minimum count flexibility
    base_vals = _validation_to_vals(base_output)
    cur_vals = _validation_to_vals(cur_output)
    allowed = _get_allowed_differences(base_vals, threshold, default_min_flex)
    results = {"Steps": {}}
    details = []
...
def _get_allowed_differences(vals, threshold, default_min_flex):
    """Retrieve allowed number of changes relative to TP counts.
    """
    allowed = {}
    for (sample, caller, vartype, metric), count in vals.items():
        if metric == "tp":
            allowed[(sample, caller, vartype)] = max(int(count * threshold), default_min_flex)
    return allowed
...
def _validation_to_vals(val_output):
    ...
    with open(val_output) as in_handle:
        ...
        for val in (dict(zip(header, xs)) for xs in reader):
            curkey = (val["sample"], val["caller"], val["variant.type"], val["category"])
